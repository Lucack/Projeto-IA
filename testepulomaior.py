...
def avaliarGenomas(genomas, config):
    ...
    for genoma_id, genoma in genomas:
        ...
        while not cabou:
            ...
            acoes_especificas = [131, 130, 130, 386]
...
checkpoint_dir = 'checkpointPULOMAIOR'  # Ou onde seus arquivos de checkpoint estão armazenados
checkpoint_files = [f for f in os.listdir(checkpoint_dir) if f.startswith('checkpoint-PULOMAIOR-')]
...
populacao.add_reporter(neat.Checkpointer(10,filename_prefix="checkpoint-PULOMAIOR-"))

# Executando a evolução
ganhador = populacao.run(avaliarGenomas, 50)

# Salvando o melhor genoma
with open('checkpointPULOMAIOR/ganhador.pkl', 'wb') as output:
